...
有状态的示例统一使用 SQLite `:memory:`，用于接近正式 Store 的行为，同时不会在工作区生成数据库文件；Integrations 示例只模拟外部 Client。
...
在 VS Code 中安装推荐的 `Code Runner` 扩展后，直接打开 `examples/*.ts`、`packages/**/*.ts` 或 `apps/**/*.ts`，点击右上角 `Run Code` 即可运行。仓库设置使用 Code Runner 的 `$workspaceRoot` 和 `$fullFileName` 变量，从根目录的 `vite-node` 执行当前文件，并自动读取根目录 `.env`；OpenRouter 示例仍需要配置 `OPENROUTER_API_KEY` 和 `OPENROUTER_MODEL`。需要注意：带未实现依赖或启动副作用的生产入口仍应使用对应 package 的测试/构建命令。

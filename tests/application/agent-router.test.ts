...
import { AgentRouter, type AgentRoutingCandidate } from "../../apps/fastmpa/src/application/dispatch/agent-router.js";

const candidates: readonly AgentRoutingCandidate[] = [
  { agentId: "research", name: "Research", role: "research", capabilities: ["search"], toolNames: [] },
  { agentId: "writer", name: "Writer", role: "writing", capabilities: ["write"], toolNames: [] },
];
...
describe("AgentRouter", () => {
  it("uses only explicitly mentioned active candidates without calling the model", async () => {
    ...
    const result = await router.route({
      workspaceId: "w", conversationId: "c", messageId: "m", body: "@Research",
      recentContext: [], candidates, maxAgents: 3, fallbackAgentId: "research",
  ...
  it("accepts valid multi-agent structured output and caps it at five", async () => {
    ...
    const result = await router.route({
      workspaceId: "w", conversationId: "c", messageId: "m", body: "help",
      recentContext: [], candidates, maxAgents: 5, fallbackAgentId: "research",
    });
  ...
  it("falls back on invalid, empty, or failed router output", async () => {
    for (const model of [
      ...
      { complete: async () => ({ type: "text" as const, content: JSON.stringify({ assignments: [] }) }) },
      ...
      expect(result).toMatchObject({ source: "fallback", selectedAgentIds: ["research"] });
    }
  });
});

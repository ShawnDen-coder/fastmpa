...
`fastmpa run "任务"` 会创建 Workspace Participant/Conversation，写入用户消息，生成确定性 Run ID，持久化并执行 Run，再把 Agent 回复写回 Conversation。Runtime 的 `enqueueIdempotent()` 返回 `created` 或 `existing`，Scheduler 和 Tool Pipeline 已归入 Runtime 公共边界。

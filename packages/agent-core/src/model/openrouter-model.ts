...
function readResponseBody(value: unknown): OpenRouterResponseBody {
  if (!value || typeof value !== "object") {
    throw new ModelExecutionError(
      "invalid_response",
      "OpenRouter returned an invalid JSON body",
    );
  }
...
function httpError(status: number, detail: string): ModelExecutionError {
  ...
  if (status === 401 || status === 403) {
    return new ModelExecutionError("authentication_failed", message, {
      status,
    });
  }
  if (status === 408) {
    return new ModelExecutionError("timeout", message, {
      status,
      retryable: true,
    });
  }
  if (status === 429) {
    return new ModelExecutionError("rate_limited", message, {
      status,
      retryable: true,
    });
  }
...
export class OpenRouterModel implements ModelAdapter {
  ...
  public async complete(
    ...
    const toolCalls = rawToolCalls.map(
      (toolCall): ToolCall => ({
        id: toolCall.id,

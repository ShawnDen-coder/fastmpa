...
function makeRun(overrides: Partial<AgentRun> = {}): AgentRun {
  return {
    runId: "run-1", status: "queued", attempt: 1, version: 0,
    createdAt: "2026-08-28T00:00:00.000Z", ...overrides,
  };
}
  ...
  it("rolls back the Run when the combined transition is invalid", async () => {
    ...
    try {
      ...
      const event: RuntimeEvent = {
        runId: "run-1", sequence: 0, type: "run_completed",
        occurredAt: "2026-08-28T00:00:00.000Z",
      };

      await expect(
        store.transitionWithEvent(
          "run-1", 0, makeRun({ status: "completed", version: 1 }), event,
        ),
